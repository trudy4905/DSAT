...
NATIVE_API void __stdcall
Engine_SetProgressCallback(EngineProgressCallback callback) {
 ...
 * ------------------------------------------------------------------------- */
NATIVE_API int32_t __stdcall Engine_ProcessDataArray(const double *inputData,
                                                     double *outputData,
                                                     int32_t dataLength,
                                                     double multiplier) {
  if (!inputData || !outputData || dataLength <= 0)
    return 0;
 ...
 * ------------------------------------------------------------------------- */
NATIVE_API int32_t __stdcall
Engine_AnalyzeDocumentOverlay(const wchar_t *filePath,
  ...
  if (!analyzer) {
    strncpy_s(outResult->detectedFormat, sizeof(outResult->detectedFormat),
              "UNKNOWN", _TRUNCATE);
    strncpy_s(outResult->statusMessage, sizeof(outResult->statusMessage),
              "지원하지 않는 확장자", _TRUNCATE);
    return 1;
  }
 ...
 * ------------------------------------------------------------------------- */
NATIVE_API int32_t __stdcall
Engine_InspectForensicImage(const wchar_t *imagePath,
                            ImageInspectionOutput *outResult) {
  char logBuf[512];
  sprintf_s(logBuf, "Engine_InspectForensicImage ENTRY: %ls",
            imagePath ? imagePath : L"(null)");
  EngineStatusManager::GetInstance().LogMessage(1, logBuf);

  if (!imagePath || !outResult)
    return -1;
...
NATIVE_API int32_t __stdcall Engine_ExtractDocumentFilesFromImage(
    const wchar_t *imagePath, const wchar_t *tempExtractDir,
    int32_t includeDeleted, ImageScanProgressCallback callback,
    int32_t *outExtractedCount) {
  if (!imagePath || !tempExtractDir)
    return -1;
  if (outExtractedCount)
    *outExtractedCount = 0;
  ...
  TskFileSystemModule fsModule;
  bool ok = fsModule.ExtractDocuments(
      reader.get(), wTempDir, includeDeleted != 0, callback, outExtractedCount);
